...
export function useCreateBooking() {
  ...
  return useMutation({
    mutationFn: (payload: BookingAdminFormValues) =>
      bookingAdminApi.createBooking(payload),
    onSuccess: async () => {
...
export function useUpdateBooking() {
  ...
  return useMutation({
    mutationFn: ({
      ...
      id: string;
      payload: Partial<BookingAdminFormValues>;
    }) => bookingAdminApi.updateBooking(id, payload),
...
export function useDeleteBooking() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (id: string) => bookingAdminApi.deleteBooking(id),
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: bookingsKeys.all });
    },
  });
