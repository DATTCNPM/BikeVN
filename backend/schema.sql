...
CREATE TABLE `users` (
  ...
  `branch_id` varchar(36) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT 'Branch where user works (for staff/manager)',
...
LEFT JOIN branches b ON u.branch_id = b.id -- Đổi từ JOIN sang LEFT JOIN
WHERE r.name IN ('employee', 'manager', 'admin'); -- Thêm 'admin' vào nếu muốn xem cả admin
